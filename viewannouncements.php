...
if ($student_username) {
    ...
    if ($team_number_result) {
        ...
        $announcement_sql = "SELECT id, announcement_content, file_name, created_at, comments FROM announcements WHERE selected_team = '$team_number' ORDER BY created_at DESC";
        ...
        if ($announcement_result) {
            if (mysqli_num_rows($announcement_result) > 0) {
                ...
                while ($announcement_row = mysqli_fetch_assoc($announcement_result)) {
                    ...
                    echo '<h5 class="card-text">' . $announcement_row['announcement_content'] . '</h5>';
...
if (!empty($announcement_row['comments'])) {
    echo '<label for="toggleComments" style="cursor: pointer;"><b>View Comments</b></label>';
    echo '<input type="checkbox" id="toggleComments" style="display: none;">'; // Hidden checkbox input
    echo '<div class="comments-content" style="display: none;">'; // Hidden initially
    echo '<p><b>Comments:</b></p>';
    
    $commentsArray = explode("\n", $announcement_row['comments']); // Split comments into an array based on newline
...
echo '</div>';
                    ...
                    echo '<form method="post" action="add_announcement_comment.php">';
